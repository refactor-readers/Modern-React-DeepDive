...
>Q2. 브라우저 이벤트 루프 관점에서 볼 때, 리액트의 상태 업데이트(setState)에 의한 리렌더링은 마이크로 태스크인가요, 매크로 태스크인가요?
...
자바스크립트는 객체가 열려 있는 구조이며 덕타이핑(duck typing)으로 객체를 비교해야 하는 특징이 있다
> 덕타이핑이란? 객체의 타입이 클래스 상속, 인터페이스 구현 등으로 결정되는 것이 아니고
어떤 객체가 필요한 변수와 메서드만 지니고 있다면 그냥 해당 타입에 속하도록 인정해 주는 것

타입스크립트는 자바스크립트의 슈퍼셋이기 때문에 똑같이 덕타이핑으로 객체를 비교한다

이러한 이유로 Object.keys같은 경우는 string[]를 반환하도록 고정되어 있다
