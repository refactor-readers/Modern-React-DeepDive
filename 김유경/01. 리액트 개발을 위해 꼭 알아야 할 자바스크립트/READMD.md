...
### 💡 Deep Dive : 변경 가능한 형태로 저장된다는 것은 무슨 의미일까?

- 변수 칸에는 실제 데이터가 아니라 **"참조값"**만 저장을 하고, 진짜 변수의 **"값"**은 Heap 같은 넓은 메모리 영역에 `키-값 테이블(프로퍼티 맵)`형태로 저장된다.

```ts
const user = { name: "Yoo", age: 29 };
const other = user;
```

개념적으로 봤을 때

- 변수 `user`은 : `0x1000` 같은 주소를 참조
- 변수 `other`: `user` 과 같은 주소 참조
- 주소 `0x1000` 위치(heap 쪽)에 프로퍼티 테이블
...
- other가 들고 있는 참조 `0x1000`을 따라가서 그 위치의 "age"라는 키의 value 칸만 29에서 30으로 바꿈
- 그래도 `other`이 들고 있는 참조값 `0x1000`은 그대로 유지되고 참조가 가리키는 테이블의 항목만 수정되기 때문에 변경 가능한 형태로 저장된다고 하는 것이다.
