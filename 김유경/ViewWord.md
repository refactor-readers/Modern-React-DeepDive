...
#### 더블 버퍼링과 해당 용어들을 연결해본다면?

리액트는 Current 트리와 Work-in-Progress 트리가 존재하고, WIP를 미리 업데이트해서 완료되면 포인터만 바꾼다는 사실을 우리는 알죠?

그렇다면, 이 개념과 앞서 살펴본 훅의 실행 시점, Commit Phase와 Render Phase을 연결해보겠습니다.

상태 변화가 끝났다고 가정할 때

1. WIP 트리 완성: 계산 완료 ~!
2. 포인터 전환 : 이제 WIP 트리가 새로운 Current 트리가 됨
3. DOM에 반영 : 리액트가 브라우저 DOM 노드들을 수정함 (이때 useLayoutEffect가 실행)
   => 하지만 브라우저가 화면을 새로 고치기 전이라 우리 눈에는 아직 이전 화면
4. Browser Paint : 브라우저가 수정된 화면을 픽셀로 그려서 보이게 됨 !
5. Post-Paint: 화면이 바뀐 것을 확인한 후 useEffect 실행
